...
const buildSymbolInsights = (
  ...
  for (const holding of holdings) {
    ...
    const pnl = holding.pnl === null || holding.pnl === undefined ? null : holding.pnl

    if (current) {
      current.value += currentValue
      current.invested += investedAmount
      current.pnl = current.pnl === null || pnl === null ? null : current.pnl + pnl
      current.lots += 1
      continue
    }
  ...
  return Array.from(grouped.values())
    .map((item) => {
      const pnlPctRaw = item.pnl !== null && item.invested > 0 ? (item.pnl / item.invested) * 100 : null
      return {
        label: normalizeDisplaySymbol(item.symbol),
        symbol: item.symbol,
        name: item.name,
        value: item.value,
        weight: totalCurrentValue > 0 ? (item.value / totalCurrentValue) * 100 : 0,
        pnl: item.pnl !== null && Number.isFinite(item.pnl) ? item.pnl : null,
