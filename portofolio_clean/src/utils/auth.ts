...
const normalizeProfile = (user?: Partial<AuthUserProfile> | null): AuthUserProfile | null => {
  ...
  const proUntil = user?.proUntil || user?.pro_until || user?.planExpiresAt || null
  const subscriptionStatus = user?.subscriptionStatus || user?.subscription_status || null
  const hasFutureProUntil = proUntil ? new Date(proUntil).getTime() > Date.now() : false
  const hasActiveSubscription = String(subscriptionStatus || '').toLowerCase() === 'active'
  const isPro = user?.isPro === true || user?.is_pro === true || hasActiveSubscription || hasFutureProUntil
  const plan = String(user?.plan || '').toLowerCase() === 'pro' || isPro ? 'pro' : 'free'
  const planExpiresAt = user?.planExpiresAt || proUntil
...
export const getStoredProfile = (): AuthUserProfile | null =>
  normalizeProfile({
    ...
    plan: window.localStorage.getItem(USER_PLAN_KEY) || 'free'
  })
