...
function parseLogamMulia(html) {
  ...
  const section =
    start >= 0
      ? text.slice(start, end > start ? end : Math.min(text.length, start + 1500))
      : text;
...
async function getAntamLive() {
  const response = await axios.get(LOGAM_MULIA_URL, {
    timeout: 12000,
    headers: {
      "User-Agent":
  ...
  const latestDate = toDateOnly(
    parsed.dateText ? new Date(parsed.dateText) : new Date()
  );
